  ...
  return (dispatch: () => void, getState) => {
    ...
    rix(connection).getTableRows(query).then((results) => {
      ...
      if (contract && contract['arisen.token']) {
        const supply = parseFloat(contract['arisen.token'].RIX.supply);
        // yearly inflation
      ...
      const data = rows
        .filter((p) => (p.producer_key !== 'RIX1111111111111111111111111111111114T1Anm'))
        .map((producer) => {
...
export function getProducersInfo(previous = false) {
  return (dispatch: () => void, getState) => {
    ...
    if (connection.chain !== 'rix-mainnet') return;
    const query = {
      json: true,
      code: 'producerjson',
      scope: 'producerjson',
