...
class WalletStatusBalances extends Component<Props> {
  ...
  render() {
            ...
            <Table size="small">
              <Table.Body>
                <Table.Row>
                  ...
                  <Table.Cell>{(tokens.RIX) ? tokens.RIX.toFixed(4) : '0.0000'} RIX</Table.Cell>
                </Table.Row>
                <Table.Row>
                  <Table.Cell>{t('wallet_status_balances_staked_to_self')}</Table.Cell>
                  <Table.Cell>{totalStakedToSelf.toFixed(4)} RIX </Table.Cell>
                </Table.Row>
                <Table.Row>
                  <Table.Cell>{t('wallet_status_balances_staked_to_others')}</Table.Cell>
                  <Table.Cell>{totalStakedToOthers.toFixed(4)} RIX </Table.Cell>
                </Table.Row>
                {(refundDate)
                  ? (
                    <Table.Row>
                      ...
                      <Table.Cell>
                        ...
                        {totalBeingUnstaked.toFixed(4)} RIX (<TimeAgo date={refundDate} />)
                      </Table.Cell>
                    </Table.Row>
                  )
                  : false
                ...
                <Table.Row>
                  ...
                  <Table.Cell>{totalTokens.toFixed(4)} RIX</Table.Cell>
                </Table.Row>
                <Table.Row>
    ...
    forEach(tokens, (amount, token) => {
      if (token === 'RIX' || watchedTokens.indexOf(token) === -1) return;
