        ...
        <Segment padding="true" basic>
          ...
          <Segment.Group>
            {(netDifference > 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="green">
                    <Icon name="wifi" />{t('about_to_stake_to_net')} {netDifference.toFixed(4)} RIX
                  </font>
                  <Header.Subheader>
            ...
            {(netDifference < 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="red">
                    <Icon name="wifi" />{t('about_to_unstake_from_net')} {(-netDifference).toFixed(4)} RIX
                  </font>
                  <Header.Subheader>
                    ({t('will_have')} {netAmount.toFixed(4)} {t('rsn_in_net_after')})
                  </Header.Subheader>
            ...
            {(cpuDifference > 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="green">
                    <Icon name="microchip" />{t('about_to_stake_to_cpu')} <b>{cpuDifference.toFixed(4)} RIX</b>
                  </font>
                  <Header.Subheader>
            ...
            {(cpuDifference < 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="red">
                    <Icon name="microchip" />{t('about_to_unstake_from_cpu')} <b>{(-cpuDifference).toFixed(4)} RIX</b>
                  </font>
                  <Header.Subheader>
                    ({t('will_have')} {cpuAmount.toFixed(4)} {t('rsn_in_cpu_after')})
                  </Header.Subheader>
          ...
          {(unstakingWhenAmountBeingUnstaked) ? (
            <Message
              ...
              warning="true"
            >
              {t('have_already_unstaked')} {statsFetcher.totalBeingUnstaked().toFixed(4)} RIX {t('unstaking_will_be_reset')}
            </Message>
          ) : ''}
